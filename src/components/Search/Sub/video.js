...
export default class Video extends React.Component {
	...
	componentWillMount(){
		this.getAllDatas(this.props.searchData.search,1,12,AllRead.getCookie('NUMBERID'))
	};
	getAllDatas(data,pageNo,perPage,numberId){
	 var data={context:data,type:1,pageNo:pageNo,perPage:perPage};
	 	if(numberId==''){
			var ori_data={
				data:data
			}
	...
	.then(response => response.json()).then(json => {
		if(json.message=='参数不能为空'){
			...
			this.setState({getVideos:json.data.videoList,total_get:json.data.total,totalNum:json.data.total,end:true})
		}
	});
	}
	PagenationSearch_get(page){
		this.getAllDatas(this.props.searchData.search,page,12,AllRead.getCookie('NUMBERID'))
	...
	render() {
        ...
        <div class="video">
            <div class='tit'>找到相关视频共 <span>{this.state.totalNum}</span> 个</div>
            <div class="video_cont clearfix" style={{marginTop:'16px'}}>
							<ReMen datas={this.state.getVideos} end={this.state.end}/>
						</div>
        </div>
