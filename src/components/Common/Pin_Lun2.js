...
import TanKuang from '../Common/TanKuang'
  ...
  goDetail(){
     let searchId=this.props.dynamicId;
	...
	render() {
      ...
      <div style={{backgroundColor:'rgb(240,240,240)'}}>
			<div class="pinlun" style={{backgroundColor:'rgb(240,240,240)'}}>
				{/*这是单独的评论页*/}
					...
					<div class="contents">
						<div class="detail_post clearfix">
							<div class="avatar"><img src={this.state.headPath?this.state.headPath+'!GDSIZE':'/src/assets/images/filterBg.png'}/></div>
            ...
            {this.state.pinlun_count.length!=0
              ?this.state.pinlun_count.map((item,index)=>(
                <div key={index}>
                  {index<4
                  ?
    							<div  class="pinlun_count"><ReMenDetailMFirst postNum={this.state.postNum}  postBg2={this.state.postBg2} textChange2={this.textChange2.bind(this)}  fabu_content={this.fabu_post.bind(this,item.commentId,2,item.nickName)} fabu_content2={this.fabu_post.bind(this)} datas={item} width_line={this.props.width_line} /></div>
                  :''
                  }
                </div>
              ))
      ...
      {this.state.totalNum>0?<div class="more_pinglun" onClick={this.goDetail.bind(this)}>查看更多评论>></div>:''}
      </div>
		);
	};
}
